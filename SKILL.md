...
- **Aurora layer** (alive): a low-chroma Blue Hour radial gradient in oklch behind every surface. Glass sits on it. Limits: chroma ≤ 0.22, ≤ 5 lobes, **never bare text inside the band** — and **glass does not rescue a coloured hue there.** Measured, every material lands 2.6–3.0:1 in the band and the thicker ones are *worse*, because translucency passes the gradient through and `blur(50px)` averages neighbouring bright lobes *in*. **Coloured ink needs an OPAQUE ground** (`--canvas`, measured 6.10:1 where glass measured 2.87:1). Glass stays fine for primary/secondary text, which is luminance against its own material.
